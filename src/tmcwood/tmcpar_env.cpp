...
{// create file list from all lsp-files in the given dir list
  ...
  for (short uu=0;uu<numPar;uu++)
	...
	{
	  mydir = *p;
	  if (strlen(mydir.c_str()) == 0) {
	    printf("zero length directory name\n");
	    continue;
	  }
	  ...
	  if( (dip=opendir(mydir.c_str()))==NULL )
	    {
	      fprintf(stderr,"\n%s %s\n","error: cannot open directry", mydir.c_str());
	      goto hFileDone;
	    }
...
int Load_FncNames(const char *filename)
{
  ...
  while (fgets(buf,500,fp) !=NULL)
	...
	{
	  fprintf(stderr,"warning: ignored string: %s\n", buf);
	}
    }
  fclose(fp);
  return 0;
