  ...
  */
  ...
  while (TmcFileList.IsFileStackEmpty()==false)
      ...
      if (p!=TmcSymbolTable.nil())
	{
      ...
      if (tmclsp_in == NULL) {
	fprintf(stderr, "error: open error %s %s:%d\n", buffer_l, __FILE__,__LINE__);
...
fprintf(stderr, "error: open error %s\n", string_hash_file_name_in);
  ...
  std::string fn=TmcFileList.InitIterateFinalList();
  fprintf(stderr,"FinalList: %s\n", fn.c_str());  

  if (Compiler.C_output)
    if (bSingleFile==false)
      stdhead_file = fopen(stdhead_file_name, "w");
	
  fprintf(stderr,"LSP CONVERSION...\n");
  fflush(stderr);
